...
class Settings(BaseModel):
    ...
    default_model: str = os.getenv("DEFAULT_MODEL", "gpt-5-nano-2025-08-07")
...
async def download_s3_files_async(file_paths: List[str], bucket: Optional[str]) -> Path:
    ...
    log.info(f"S3 Configuration:")
    log.info(f"  Region: {region}")
    log.info(f"  AWS_ACCESS_KEY_ID: {'***' + access_key[-4:] if access_key else 'NOT SET'}")
    log.info(f"  AWS_SECRET_ACCESS_KEY: {'***' if secret_key else 'NOT SET'}")
    ...
    async with session.client("s3", config=cfg) as s3:
        ...
        async def _dl(path: str):
            ...
            print(f"Downloading {clean_path} to {local_path}, {bucket_name}")
            try:
